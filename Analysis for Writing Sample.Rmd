...
diff_df <- means %>%
  ...
  mutate(
    ...
    Treatment = factor(
      ...
      levels = rev(1:6),
      labels = rev(c(
        "Treatment 1:\nLegal, Follow Through, Success",
        "Treatment 2:\nLegal, Follow Through, Failure",
        "Treatment 3:\nLegal, Back Down",
        "Treatment 4:\nIllegal, Follow Through, Success",
        "Treatment 5:\nIllegal, Follow Through, Failure",
        "Treatment 6:\nIllegal, Back Down"
      ))
    )
  )
...
# Pairwise significance between all treatments (including control)
...
inconsistency_df <- tibble(
  Condition = factor(
    c("Internationally Lawful", "Internationally Unlawful"),
    levels = c("Internationally Lawful", "Internationally Unlawful")
  ),
...
m0 <- means %>% filter(Treatment == 0)  # Control: stay out
m2 <- means %>% filter(Treatment == 2)  # Legal, follow-through, failure
m5 <- means %>% filter(Treatment == 5)  # Illegal, follow-through, failure
...
m1 <- means %>% filter(Treatment == 1)  # Legal, follow-through, success
m2 <- means %>% filter(Treatment == 2)  # Legal, follow-through, failure
m4 <- means %>% filter(Treatment == 4)  # Illegal, follow-through, success
m5 <- means %>% filter(Treatment == 5)  # Illegal, follow-through, failure
...
extract_coef <- function(mod_list, term_name) {
  purrr::imap_dfr(mod_list, ~ {
    broom::tidy(.x) %>%
      ...
      dplyr::transmute(
        Treatment = .y,          # name from the list (e.g. "Treatment 1: ...")
        estimate  = estimate,
        se        = std.error
      )
  })
...
treat_levels <- rev(names(mods_by_treat))  # Control at top, T6 at bottom
...
x_lim <- ceiling(x_lim * 10) / 10  # clean axis (nearest 0.1)
...
treat_levels <- rev(names(mods_voting_by_treat))  # Control at top, T6 at bottom
...
x_lim <- max(abs(all_bounds))
x_lim <- ceiling(x_lim * 10) / 10  # round up to nearest 0.1 for a clean axis
